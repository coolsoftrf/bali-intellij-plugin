...
public class SetterFieldProcessor extends AbstractFieldProcessor {
  ...
  public PsiMethod createSetterMethod(@NotNull PsiField psiField, @NotNull PsiClass psiClass, @NotNull String methodModifier) {
    ...
    if (null != methodParameterModifierList) {
      AbstractProcessor.copyCopyableAnnotations(psiField, methodParameterModifierList, LombokUtils.BASE_COPYABLE_ANNOTATIONS);
      AbstractProcessor.copyOnXAnnotations(setterAnnotation, methodParameterModifierList, "onParam");
    }

    final PsiModifierList modifierList = methodBuilder.getModifierList();
    AbstractProcessor.copyCopyableAnnotations(psiField, modifierList, LombokUtils.COPY_TO_SETTER_ANNOTATIONS);
    AbstractProcessor.copyOnXAnnotations(setterAnnotation, modifierList, "onMethod");
